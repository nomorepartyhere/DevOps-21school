...
void output(int c, int argc, char *argv[], FLAGS *opt);
void f_flag(FLAGS *opt);

int main(int argc, char *argv[]) {
  FLAGS opt = {0};

  int c = flags_reading(argc, argv, &opt);
  while (c <= (argc - 1)) {
    output(c, argc, argv, &opt);
    c++;
  }
...
int flags_reading(int argc, char *argv[], FLAGS *opt) {
  ...
  if (!opt->e && !opt->f) {
    if (argc > optind) {
      strcat(opt->str, argv[optind]);  // + шаблон в строку рег. выраж
    }
    optind++;  // optind - след элемент argv после флагов
  }
  if (opt->e || opt->f) {
    opt->str[strlen(opt->str) - 1] = '\0';  // удаляем посл. "|"
                                            // printf("%s\n", A->str);
  }
  return optind;
}
...
void output(int c, int argc, char *argv[], FLAGS *opt) {
  ...
  if (opt->i) {
    flag_i = REG_EXTENDED |
             REG_ICASE;  // расшир синтаксис рег.выр. и не учитывать регистр
                         // | - последовательность, если выполнилось
                         // REG_EXTENDED, то выполнится и REG_ICASE
  }
  regcomp(&reg, opt->str, flag_i);  // компиль рег. выр.
  file = fopen(argv[c], "r");
  if (file != NULL) {
    while (fgets(opt->str_regexec, BUFFER, file) != NULL) {
      index++;
      match = regexec(&reg, opt->str_regexec, 1, &start,
                      0);  // 0 если есть совпадения
      if (opt->v) {        // флаг v
        match = !match;
      }
      ...
      if ((argc - optind) > 1) {  // если больше одного файла на входе
        ...
        if (!match && opt->o) {
          char *pointer = opt->str_o;
          while ((regexec(&reg, pointer, 1, &start, 0) == 0)) {
            // найти кол-во повторений аргументов с помощью функции;
            if (!opt->h) {
              printf("%s:%.*s\n", argv[c], (int)(start.rm_eo - start.rm_so),
                     pointer + start.rm_so);
              pointer += start.rm_eo;
            } else {
              printf("%.*s\n", (int)(start.rm_eo - start.rm_so),
